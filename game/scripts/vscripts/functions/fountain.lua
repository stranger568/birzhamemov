...
function modifier_birzha_fountain_passive:OnIntervalThink()
	...
	local units = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, 900, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE + DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_ANY_ORDER, false )
...
function modifier_fountain_passive_invul:OnCreated()
	...
	if self:GetParent():IsRealHero() then
		...
		if DonateShopIsItemBought(self.player, 44) then
...
function modifier_birzha_fountain_attacked:IsPurgable()
	return false
end

function modifier_birzha_fountain_attacked:IsPurgeException()
	return false
end
