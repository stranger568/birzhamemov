...
function modifier_migi_inside_parent:OnCreated()
    ...
    if IsServer() then
        ...
        self.health_regen = self:GetCaster():GetHealthRegen()
...
function modifier_migi_inside_parent:OnDestroy()
    ...
    if self:GetCaster():HasTalent("special_bonus_birzha_migi_6") then
        ...
        self:GetCaster():RemoveModifierByName("modifier_item_uebator_active")
...
function modifier_migi_inside_parent:GetDisableHealing()
    if self.ally then return end
    return 1
end
...
function modifier_migi_weapon:OnCreated(params)
    if IsServer() then
        local pfx = ParticleManager:CreateParticle("particles/migi_test.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_migi_weapon:OnIntervalThink()
    if IsServer() then
        ...
        if nearby_enemy_units ~= nil and #nearby_enemy_units > 0 then
            modifier_migi_weapon:OnHit(self:GetCaster(), spirit, nearby_enemy_units, 100 + self:GetCaster():GetAverageTrueAttackDamage(nil) / 2, self:GetAbility())
        end
    end
end
