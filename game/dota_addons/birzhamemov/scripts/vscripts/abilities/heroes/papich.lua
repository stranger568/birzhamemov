...
function modifier_hellfire_blast_slow:OnCreated( kv )
    self.per_damage = self:GetAbility():GetSpecialValueFor( "per_damage" ) + self:GetCaster():FindTalentValue("special_bonus_birzha_papich_1")
            ...
            if self:GetAbility():IsFullyCastable() then
                ...
                if self:GetParent():HasTalent("special_bonus_birzha_papich_5") and (self:GetParent():FindAbilityByName("Papich_HellFire_Blast"):GetLevel()>0) then
                    ...
                    for _,enemy in pairs(enemies) do
                        LaunchWraithblastProjectile(self:GetParent(), self:GetAbility(), enemy, enemy, false)
                    end
                end
            end             
        end
...
function modifier_Papich_in_solo:GetModifierPreAttack_CriticalStrike( params )
    ...
    if self.attack_chance >= RandomInt(1, 100) then
        ...
        if IsUnlockedInPass(self:GetCaster():GetPlayerID(), "reward67") then
    ...
    if chance >= RandomInt(1, 100) then
        self:GetParent():StartGesture(ACT_DOTA_ATTACK_EVENT)
        self:GetParent():StopSound("papichwherecrit")
        self:GetParent():EmitSound("papichsolo")
        self.attach_sound = 0
        return crit
    end
    return 0
