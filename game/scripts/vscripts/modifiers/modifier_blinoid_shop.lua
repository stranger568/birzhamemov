...
LinkLuaModifier("modifier_blinoid_shop_knockback", "modifiers/modifier_blinoid_shop", LUA_MODIFIER_MOTION_BOTH)
...
function modifier_blinoid_shop:OnIntervalThink()
	...
	if #targets > 0 then
		...
		self:GetParent():RemoveModifierByName("modifier_blinoid_shop_knockback")
		self:GetParent():AddNewModifier( self:GetParent(), nil, "modifier_blinoid_shop_knockback", { duration = 0.25, distance = 300, direction_x = direction.x, direction_y = direction.y, IsFlail = false, } )
		if not self:GetParent():HasModifier("modifier_blinoid_shop_sound") then
			...
			EmitSoundOn("blinoid_attack", self:GetParent())
		end
	end
end
...
function modifier_blinoid_shop_move:OnDestroy()
	if not IsServer() then return end
	local forward = self:GetParent():GetForwardVector()
    forward.z = 0
...
modifier_blinoid_shop_knockback = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_blinoid_shop_knockback:IsHidden()
	return true
end

function modifier_blinoid_shop_knockback:IsPurgable()
	return false
end

function modifier_blinoid_shop_knockback:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end
--------------------------------------------------------------------------------
-- Initializations
function modifier_blinoid_shop_knockback:OnCreated( kv )
	if IsServer() then
...
function modifier_blinoid_shop_knockback:OnRefresh( kv )
	if not IsServer() then return end
end

function modifier_blinoid_shop_knockback:OnDestroy( kv )
	if not IsServer() then return end
	...
	self:GetParent():AddNewModifier(self:GetParent(), nil, "modifier_blinoid_shop_move", {duration = 1})
...
--------------------------------------------------------------------------------
-- Setter
function modifier_blinoid_shop_knockback:SetEndCallback( func ) 
	self.EndCallback = func
end

function modifier_blinoid_shop_knockback:UpdateHorizontalMotion( me, dt )
...
function modifier_blinoid_shop_knockback:OnHorizontalMotionInterrupted()
	if IsServer() then
		self.interrupted = true
		self:Destroy()
	end
...
function modifier_blinoid_shop_knockback:UpdateVerticalMotion( me, dt )
	-- set time
...
function modifier_blinoid_shop_knockback:OnVerticalMotionInterrupted()
	if IsServer() then
		self.interrupted = true
		self:Destroy()
	end
