...
function modifier_sobolev_prank:OnDestroy( kv )
	if not IsServer() then return end
	if self:GetCaster():HasTalent("special_bonus_birzha_sobolev_1") then self:GetParent():RemoveNoDraw() return end
...
function Sobolev_Egoism:OnSpellStart()
	...
	if IsUnlockedInPass(self:GetCaster():GetPlayerID(), "reward95") then
