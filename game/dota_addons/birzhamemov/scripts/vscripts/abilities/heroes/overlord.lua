...
function overlord_fireball:OnSpellStart()
    ...
    Timers:CreateTimer(0.25, function()
         CreateModifierThinker( caster, self, "modifier_overlord_fireball", { duration = self.duration }, point, caster:GetTeamNumber(), false )
    end)
...
function modifier_overlord_fireball:OnDestroy()
    if not IsServer() then return end
    self:GetParent():StopSoundOn("n_black_dragon.Fireball.Target", self:GetParent())
end
...
function modifier_overlord_shut_debuff:OnIntervalThink()
    if not IsServer() then return end
    local damage = self:GetAbility():GetSpecialValueFor("damage") + (self:GetCaster():GetAverageTrueAttackDamage(self:GetParent()) / 2)
    local damageTable = 
    {
        victim = self:GetParent(),
        attacker = self:GetCaster(),
