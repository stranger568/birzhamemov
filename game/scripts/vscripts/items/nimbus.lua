...
function modifier_item_nimbus_active:GetModifierTotal_ConstantBlock(params)
    if IsServer() then
        ...
        if params.damage > 0 and bit.band(params.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS and bit.band(params.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) ~= DOTA_DAMAGE_FLAG_REFLECTION then
            ...
            if not self.carapaced_units[ params.attacker:entindex() ] then
				...
				params.attacker:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_birzha_stunned", {duration = stun_duration * (1 - params.attacker:GetStatusResistance())})
				if self:GetAbility():GetAbilityName() == "item_nimbus_lapteva_2" then
