...
function modifier_overlord_spell_4_buff:OnDestroy()
    if IsServer() then
        self:GetParent():Kill(self.ability_killer, self.original_killer)
        if self:GetParent():IsAlive() then
            self:GetParent():RemoveModifierByName("modifier_Overlord_spell_10_invul")
            self:GetParent():RemoveModifierByName("modifier_Overlord_spell_10_buff")
            self:GetParent():RemoveModifierByName("modifier_Overlord_spell_7_buff")
            self:GetParent():RemoveModifierByName("modifier_item_uebator_active")
            self:GetParent():RemoveModifierByName("modifier_LenaGolovach_Radio_god")
            self:GetParent():RemoveModifierByName("modifier_kurumi_god")
            self:GetParent():RemoveModifierByName("modifier_Felix_WaterShield")
            self:GetParent():RemoveModifierByName("modifier_ExplosionMagic_immunity")
            self:GetParent():RemoveModifierByName("modifier_item_nimbus_active")
            self:GetParent():RemoveModifierByName("modifier_haku_help")
            self:GetParent():RemoveModifierByName("modifier_item_birzha_blade_mail_active")
            self:GetParent():RemoveModifierByName("modifier_invulnerable")
            self:GetParent():RemoveModifierByName("modifier_item_aeon_disk_buff")
            self:GetParent():RemoveModifierByName("modifier_papich_reincarnation_wraith_form")
            self:GetParent():Kill(self.ability_killer, self.original_killer)
        end
    end
    self.caster = nil
        ...
        if self:GetParent():FindAllModifiersByName(self:GetName())[1] == self then
            if kv.damage > 0 and bit.band(kv.damage_flags, DOTA_DAMAGE_FLAG_HPLOSS) ~= DOTA_DAMAGE_FLAG_HPLOSS then
                ...
                if kv.damage < self.damage_absorb then
                    ...
                    Timers:CreateTimer(0.06, function() self:Destroy() end)
...
function modifier_Overlord_passive:DeclareFunctions()
    local decFuncs = {MODIFIER_PROPERTY_MANA_BONUS, MODIFIER_EVENT_ON_ATTACK}
...
function Overlord_spell_ultimate:OnSpellStart()
    ...
    print(health_damage,mana_damage,damage)
...
function modifier_Overlord_spell_ultimate:OnIntervalThink(table)
    ...
    if self.alive_time <= 0 then
        self:GetParent():Destroy()
        self:Destroy()
        local thinker = CreateModifierThinker(self:GetCaster(), self:GetAbility(), "modifier_Overlord_spell_ultimate_aura_thinker", {duration = 8.1, damage = self.damage}, Vector(0,0,0), self:GetCaster():GetTeamNumber(), false)
    end
end
...
function modifier_Overlord_spell_ultimate_aura:Custom_AllHealAmplify_Percentage()
    return -50
end
