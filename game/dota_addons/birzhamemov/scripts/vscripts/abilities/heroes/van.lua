...
function modifier_van_swallowmycum_debuff:OnCreated(kv)
    ...
    if self.unique == 1 then
        if self:GetCaster():GetAgility() >= self:GetParent():GetAgility() then
            ...
            ApplyDamage({ victim = self:GetParent(), attacker = self:GetCaster(), damage = self:GetCaster():GetAgility(), damage_type = DAMAGE_TYPE_PURE })
        else
            ...
            ApplyDamage({ victim = self:GetCaster(), attacker = self:GetParent(), damage = self:GetParent():GetAgility(), damage_type = DAMAGE_TYPE_PURE })
        end
    end
end
...
function modifier_van_swallowmycum_illusion:IllThink()
