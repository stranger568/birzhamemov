...
function Felix_WaterStream:OnProjectileHit(target, location, extra_data)
	if not IsServer() then return end
	local damage = self:GetSpecialValueFor("damage") + (self:GetCaster():GetIntellect() * self:GetSpecialValueFor("multi"))
	local heal = self:GetSpecialValueFor("heal") + (self:GetCaster():GetIntellect() * self:GetSpecialValueFor("multi"))
