...
function modifier_rat_courier_infection_unit:OnCreated(params)
    ...
    if self:GetCaster():HasShard() then
        self.destroy_attack = 5
    end
    print(self.destroy_attacks)
...
function modifier_rat_burrow:OnAttackLanded( params )
    ...
    if parent == params.attacker and target:GetTeamNumber() ~= parent:GetTeamNumber() then
        if not self.targets[target:entindex()] then
            ...
            target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_rat_burrow_debuff", {duration = debuff_duration})
            AddNewStack(2, target, self:GetParent())
            ...
            if self:GetCaster():HasTalent("special_bonus_birzha_rat_4") then
                ...
                target:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_rat_burrow_attackspeed_debuff", {duration = self:GetRemainingTime()})
            end
...
function modifier_rat_burrow_attackspeed_buff:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE 
end
...
function AddNewStack(count, target, caster)
    ...
    if target and caster then
        if target:IsRealHero() then
            ...
            target:AddNewModifier(caster, ability, "modifier_rat_passive_stack", {duration = 20})
            if target:HasModifier("modifier_rat_passive_stack") then
                local modifier = target:FindModifierByName("modifier_rat_passive_stack")
                if modifier:GetStackCount() < ability:GetSpecialValueFor("max_stack_count") then
                    modifier:SetStackCount(modifier:GetStackCount() + count)
                    local caster_modifier = caster:FindModifierByName("modifier_rat_passive_caster_attack")
                    if caster_modifier then
                        caster_modifier:SetStackCount(caster_modifier:GetStackCount() + count)
                    end
                end
                if modifier:GetStackCount() > ability:GetSpecialValueFor("max_stack_count") then
                    modifier:SetStackCount(ability:GetSpecialValueFor("max_stack_count"))
                end
            else
                local modifier = target:AddNewModifier(caster, ability, "modifier_rat_passive_stack", {duration = 20})
                if modifier then
                    modifier:SetStackCount(modifier:GetStackCount() + count)
                    local caster_modifier = caster:FindModifierByName("modifier_rat_passive_caster_attack")
                    if caster_modifier then
...
function modifier_rat_passive_caster_attack:IsHidden() return true end
...
function modifier_rat_passive_caster_attack:OnAttackLanded( params )
    ...
    if parent == params.attacker and target:GetTeamNumber() ~= parent:GetTeamNumber() then
        if self:GetParent():HasTalent("special_bonus_birzha_rat_8") then
            AddNewStack(3, target, self:GetParent())
        else
            AddNewStack(1, target, self:GetParent())
        end
    end
end

function modifier_rat_passive_caster_attack:OnDeath( params )
    if not IsServer() then return end
    if params.unit == self:GetParent() then
        ...
        local units = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO+DOTA_UNIT_TARGET_BASIC, 0, FIND_CLOSEST, false )
        for _, unit in pairs(units) do
            AddNewStack(5, unit, self:GetParent())
        end
    end
end
...
function modifier_rat_passive_caster_attack:GetModifierPhysicalArmorBonus( params )
    if self:GetParent():HasTalent("special_bonus_birzha_rat_2") then
        local stack_count = math.ceil(self:GetStackCount() / 10)
        return stack_count * 5
    end
    return 0
end
...
function modifier_rat_passive_stack:OnDestroy()
    ...
    if caster_modifier then
        caster_modifier:SetStackCount(caster_modifier:GetStackCount() - self:GetStackCount())
        if caster_modifier:GetStackCount() < 0 then
            caster_modifier:SetStackCount(0)
        end
    end

    local radius = self:GetAbility():GetSpecialValueFor("radius")
    local damage = self:GetAbility():GetSpecialValueFor("stack_damage") + self:GetCaster():FindTalentValue("special_bonus_birzha_rat_6")
    ...
    if self:GetCaster():HasTalent("special_bonus_birzha_rat_7") then
        local units_talent = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, 600, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO+DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_CLOSEST, false )
        for _, unit in pairs(units_talent) do
            print(self:GetStackCount()/2)
            AddNewStack(self:GetStackCount()/2, unit, self:GetCaster())
        end
    end
end
