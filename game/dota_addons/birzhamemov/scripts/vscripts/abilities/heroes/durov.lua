...
function Durov_AttackOnPoliceman:OnSpellStart()
    ...
    local particle = ParticleManager:CreateParticle("particles/econ/items/tinker/boots_of_travel/teleport_start_bots.vpcf", PATTACH_WORLDORIGIN, thinker)
...
--[[function modifier_Durov_Vpn_buff:OnCreated()
    if not IsServer() then return end
    self.percent = self:GetAbility():GetSpecialValueFor( "bonus_attribute" )
