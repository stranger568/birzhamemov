...
function scp682_bite:GetIntrinsicModifierName()
    return "modifier_generic_custom_indicator"
end

function scp682_bite:CastFilterResultLocation( vLoc )
    if IsClient() then
...
function scp682_bite:UpdateCustomIndicator( loc )
    ...
    ParticleManager:SetParticleControl( self.effect_cast, 1, origin + direction*(self:GetCastRange( loc, nil )) )
    ParticleManager:SetParticleControl( self.effect_cast, 6, origin + direction*point_blank )
end

function scp682_bite:DestroyCustomIndicator()
...
function scp682_bite:OnSpellStart()
    ...
    for _,target in pairs(units) do
        ...
        local damageTable = {
            victim = target,
            attacker = caster,
            damage = damage + self:GetCaster():GetAverageTrueAttackDamage(target),
