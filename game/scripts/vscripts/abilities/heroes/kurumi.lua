...
function Kurumi_Absorption:OnSpellStart()
    ...
    target:AddNewModifier(self:GetCaster(), self, "modifier_Kurumi_Absorption_debuff", {duration = 3})
...
function modifier_Kurumi_Absorption_debuff:OnDeath(params)
    ...
    if target:IsRealHero() and caster:GetTeamNumber() ~= target:GetTeamNumber() and caster:IsAlive() and target == self:GetParent() and params.attacker == self:GetCaster() then   
