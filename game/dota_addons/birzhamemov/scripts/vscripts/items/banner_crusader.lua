...
function modifier_item_banner_crusader:GetAttributes()
...
function modifier_item_banner_crusader_aura:OnCreated()
	...
	if not IsServer() then return end
	self.crimson_guard_pfx = ParticleManager:CreateParticle("particles/items2_fx/vanguard_active.vpcf", PATTACH_CUSTOMORIGIN, self:GetParent())
    ParticleManager:SetParticleControl(self.crimson_guard_pfx, 0, Vector(self:GetParent():GetAbsOrigin().x+300, self:GetParent():GetAbsOrigin().y, self:GetParent():GetAbsOrigin().z))
    ParticleManager:SetParticleControlEnt(self.crimson_guard_pfx, 1, self:GetParent(), PATTACH_ABSORIGIN_FOLLOW, nil, self:GetParent():GetAbsOrigin(), true)
end
