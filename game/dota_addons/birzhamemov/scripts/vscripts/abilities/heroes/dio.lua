...
function Dio_Kakyoin:PlayEffects1( target )
	local effect_cast = ParticleManager:CreateParticle( "particles/units/heroes/hero_earth_spirit/espirit_bouldersmash_caster.vpcf", PATTACH_WORLDORIGIN, nil )
...
function modifier_dio_TheWorld:OnCreated(keys)
    self.b_damage = self:GetAbility():GetSpecialValueFor("stand_damage") + ( 10 * (self:GetCaster():GetIntellect() / self:GetAbility():GetSpecialValueFor("bonus_damage")))
    self.b_health = self:GetAbility():GetSpecialValueFor("stand_hp")
    self.b_armor = self:GetAbility():GetSpecialValueFor("stand_armor") + self:GetCaster():FindTalentValue("special_bonus_birzha_dio_4")
    if not IsServer() then return end
