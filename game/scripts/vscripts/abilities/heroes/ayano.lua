...
function modifier_ayano_TakeACircularSaw:GetModifierDamageOutgoing_Percentage()
    if self:GetCaster():HasShard() then
        return self:GetCaster():FindTalentValue("special_bonus_birzha_ayano_1")
    end
    return self:GetAbility():GetSpecialValueFor("damage_reduced") + self:GetCaster():FindTalentValue("special_bonus_birzha_ayano_1")
end
...
function Ayano_Tranquilizer:OnSpellStart()
    ...
    if not self:GetCaster():HasScepter() then
        ...
        local targets = FindUnitsInRadius( self:GetCaster():GetTeamNumber(), self:GetCursorPosition(), nil, 200, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_DAMAGE_FLAG_NONE, 0, false )
...
function modifier_SpotTheTarget_aura:CheckState()
    if self:GetParent():HasModifier("modifier_metagame_shadow_blade") then
        return
    end
