...
function modifier_akame_obraz_illusion:OnAttackLanded( params )
    ...
    if parent == params.attacker and target:GetTeamNumber() ~= parent:GetTeamNumber() then
    	local damage_persentage = self:GetAbility():GetSpecialValueFor( "damage_one_persentage" ) / 100 * self:GetCaster():GetAverageTrueAttackDamage(target)
    	local base_damage = self:GetAbility():GetSpecialValueFor( "damage_one" )
    	local damage = base_damage + damage_persentage
    	ApplyDamage({ victim = target, attacker = self:GetParent(), damage = damage, damage_type = DAMAGE_TYPE_PHYSICAL })
        self:Destroy()
    end
end
...
function modifier_akame_attack_series_passive:OnAttackLanded( keys )
	if IsServer() then
		...
		local chance = self:GetAbility():GetSpecialValueFor("chance") + self:GetCaster():FindTalentValue("special_bonus_birzha_akame_4")
		if chance >= RandomInt(1, 100) then
			attacker:AddNewModifier(attacker, self:GetAbility(), "modifier_akame_attack_series_passive_haste", {duration = 1.5})
		end
	end
end
...
function modifier_akame_attack_series:OnIntervalThink()
	if not IsServer() then return end
	if not self.target:IsAlive() then self:OnDestroy() self:Destroy() return end
	if not self:GetCaster():CanEntityBeSeenByMyTeam(self.target) then self:OnDestroy() self:Destroy() return end
	if self.attack_count == 0 then self:Destroy() return end
	if self.attack_count == 1 then
		...
		ApplyDamage({ victim = self.target, attacker = self:GetParent(), damage = last_damage, damage_type = DAMAGE_TYPE_PHYSICAL })
	end
...
function Akame_cursed_blade:OnSpellStart()
    ...
    for _,unit in ipairs(units) do
        ...
        ApplyDamage({ victim = unit, attacker = self:GetCaster(), damage = damage, damage_type = DAMAGE_TYPE_PHYSICAL})
...
function modifier_cursed_blade_debuff:OnIntervalThink()
	...
	ApplyDamage({victim = self:GetParent(), attacker = self:GetCaster(), damage = damage, damage_type = DAMAGE_TYPE_PHYSICAL})
end
