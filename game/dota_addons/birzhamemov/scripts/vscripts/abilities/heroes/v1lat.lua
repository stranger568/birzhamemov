...
function V1lat_AiAiAi_slam:OnProjectileHit_ExtraData(target, location, data)
    if not target and self.ice_blast_ability then
        ...
        local duration      = self.ice_blast_ability:GetSpecialValueFor("frostbite_duration")
        local stun_duration      = self.ice_blast_ability:GetSpecialValueFor("duration") + self:GetCaster():FindTalentValue("special_bonus_birzha_v1lat_2")
...
function V1lat_Eminem:OnSpellStart()
    ...
    self.modifier = caster:FindModifierByName("modifier_v1lat_eminem")
end
...
function modifier_v1lat_eminem:OnIntervalThink()
    ...
    local projectile_direction = (self:GetAbility().point-self:GetCaster():GetOrigin()):Normalized()
...
function V1lat_Eminem:OnProjectileHit( target, location )
    if not IsServer() then return end
    local damage = self:GetSpecialValueFor("damage") + self:GetCaster():FindTalentValue("special_bonus_birzha_v1lat_3")
