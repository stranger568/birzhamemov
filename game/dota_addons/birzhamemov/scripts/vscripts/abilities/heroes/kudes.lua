...
function modifier_JumpInHead_arena:OnCreated(kv)
	...
	Timers:CreateTimer(0.6,function()
		...
		for k,v in pairs(entities) do
			v:AddNewModifier(caster, self:GetAbility(), "modifier_JumpInHead_arena_debuff", {Duration=duration})
		end
		...
		if v:IsAlive() then
			if ( v:GetRangeToUnit(self:GetParent()) < ( radius ) ) then
				if ( v:GetCreationTime() >= GameRules:GetGameTime() - 0.3 ) then
					...
					v:AddNewModifier(self:GetAbility():GetCaster(), self:GetAbility(), "modifier_JumpInHead_arena_debuff", {Duration=duration})
				end

				if ( not v:HasModifier("modifier_JumpInHead_arena_debuff") ) then
					...
					if v:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then
                        if v:IsMagicImmune() then
                            local mod_b = v:FindModifierByName("modifier_JumpInHead_arena_debuff")
                            if mod_b then
                                mod_b:Destroy()
                            end
                            return
                        end
						ApplyDamage({attacker = self:GetCaster(), victim = v, ability = self:GetAbility(), damage = damage, damage_type = DAMAGE_TYPE_MAGICAL})
						self.particle2 = ParticleManager:CreateParticle("particles/units/heroes/hero_mars/mars_arena_of_blood_impact.vpcf", PATTACH_ABSORIGIN_FOLLOW, v)
						ParticleManager:SetParticleControl(self.particle2, 0, v:GetAbsOrigin())
						v:EmitSound("Hero_Mars.Spear.Knockback")
				...
				if ( v:HasModifier("modifier_JumpInHead_arena_debuff") ) then
					local vpos = v:GetAbsOrigin()
					local ppos = self:GetParent():GetAbsOrigin()
					local dir = ( vpos - ppos ):Normalized()
					local rdir = ( ppos - vpos ):Normalized()
...
function modifier_Kudes_Fat:GetModifierIncomingDamage_Percentage ( params )
	return self:GetStackCount() * self.damage_armor
end
