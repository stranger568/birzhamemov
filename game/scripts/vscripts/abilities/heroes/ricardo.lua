...
function Ricardo_Zhopa:OnSpellStart()
    ...
    local damage_unit = self:GetSpecialValueFor("damage_unit") + self:GetCaster():FindTalentValue("special_bonus_birzha_ricardo_5")
...
function modifier_Ricardo_shel:OnAbilityExecuted( params )
    if IsServer() then
        ...
        local enemies = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), self:GetParent(), radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, 0, 0, false )
        if #enemies > 0 then
            for _,enemy in pairs( enemies ) do
                enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_birzha_stunned_purge", {duration = duration})
...
function modifier_Ricardo_KokosMaslo_debuff:GetModifierMagicalResistanceBonus()
    return self:GetAbility():GetSpecialValueFor("mag_resist") * self:GetStackCount()
end 
...
function modifier_Ricardo_KokosMaslo_debuff:OnTakeDamage(keys)
    if keys.attacker == self:GetCaster() and keys.unit == self:GetParent() and (not keys.inflictor or not self.non_trigger_inflictors[keys.inflictor:GetName()]) then
        local damage = (self:GetAbility():GetSpecialValueFor("bonus_damage") + self:GetCaster():FindTalentValue("special_bonus_birzha_ricardo_2")) * self:GetStackCount()
...
function modifier_Rikardo_Fire:OnCreated()
    if not IsServer() then return end
    local interval = self:GetAbility():GetSpecialValueFor("interval") + self:GetCaster():FindTalentValue("special_bonus_birzha_ricardo_1")
...
function modifier_Rikardo_Fire:OnIntervalThink()
    if self:GetCaster():HasTalent("special_bonus_birzha_ricardo_1") then
        if not talent then
            self:ForceRefresh()
            talent = true
        end
    ...
    for _,unit in pairs(targets) do
        if self:GetCaster():HasTalent("special_bonus_birzha_ricardo_4") then
            damage = unit:GetMaxHealth() * 0.03
        end
        local damage_type = DAMAGE_TYPE_MAGICAL
        if self:GetCaster():HasShard() then
            damage_type = DAMAGE_TYPE_PURE
        end 
        ApplyDamage({victim = unit, attacker = self:GetCaster(), damage = damage, damage_type = damage_type, ability = self:GetAbility()})
    end
end

LinkLuaModifier("modifier_Ricardo_Golosovanie", "abilities/heroes/ricardo.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_Ricardo_Golosovanie_slow", "abilities/heroes/ricardo.lua", LUA_MODIFIER_MOTION_NONE)
...
function Ricardo_Golosovanie:GetCastRange(location, target)
    return self:GetSpecialValueFor("radius") + self:GetCaster():FindTalentValue("special_bonus_birzha_ricardo_6")
end
...
function modifier_Ricardo_Golosovanie:OnIntervalThink()
    if not IsServer() then return end
    local damage = self:GetAbility():GetSpecialValueFor("damage")
    local radius = self:GetAbility():GetSpecialValueFor("radius") + self:GetCaster():FindTalentValue("special_bonus_birzha_ricardo_6")
    ...
    for _,unit in pairs(targets) do
        ...
        unit:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_Ricardo_Golosovanie_slow", {duration = 0.5})
...
modifier_Ricardo_Golosovanie_slow = class({})

function modifier_Ricardo_Golosovanie_slow:DeclareFunctions()
    return {
        MODIFIER_PROPERTY_MOVESPEED_BONUS_PERCENTAGE,
    }
end

function modifier_Ricardo_Golosovanie_slow:GetModifierMoveSpeedBonus_Percentage()
