...
function modifier_birzha_fountain_passive:IsHidden() return true end
function modifier_birzha_fountain_passive:GetModifierAura() return "modifier_fountain_passive_invul" end
function modifier_birzha_fountain_passive:IsAura() return true end
function modifier_birzha_fountain_passive:GetAuraRadius() return 950 end
function modifier_birzha_fountain_passive:GetAuraDuration() return 1 end
function modifier_birzha_fountain_passive:GetAuraSearchTeam() return DOTA_UNIT_TARGET_TEAM_FRIENDLY end
function modifier_birzha_fountain_passive:GetAuraSearchFlags() return DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD end
function modifier_birzha_fountain_passive:GetAuraSearchType() return DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC end
...
function modifier_birzha_fountain_passive:OnIntervalThink()
	if not IsServer() then return end
    self:GetCaster():RemoveModifierByName("modifier_fountain_aura")
    if BIRZHA_FOUNTAIN_GAME_TIMER <= 0 then
        self:StartIntervalThink(-1)
        return 
    end
	local units = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetAbsOrigin(), nil, 950, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_INVULNERABLE, FIND_ANY_ORDER, false )
...
function modifier_fountain_passive_invul:OnCreated()
    ...
    if self:GetParent():IsRealHero() then
        if DonateShopIsItemBought(self:GetParent():GetPlayerOwnerID(), 44) then
